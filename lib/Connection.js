...
Connection.prototype._login = function() {
  ...
  self._enqueue('CAPABILITY', function() {
    ...
    if (self.state === 'connected') {
      ...
      // LOGINDISABLED (RFC 3501 6.2.3, RFC 2595 5) bans the plaintext LOGIN
      // command and nothing else. SASL mechanisms are advertised separately as
      // AUTH=<mech> and remain usable, so refusing to authenticate at all here
      // would lock us out of servers that advertise, say,
      // "LOGINDISABLED AUTH=XOAUTH2" -- a perfectly usable configuration.
      //
      // Over a cleartext link we still refuse everything: a server that bans
      // plaintext LOGIN is telling us not to put credentials on the wire, and
      // an OAuth bearer token is a credential too.
      var loginDisabled = self.serverSupports('LOGINDISABLED'),
          encrypted = !!(self._sock && self._sock.encrypted),
          canSASL = encrypted || !loginDisabled;

      var cmd;
      if (canSASL && self.serverSupports('AUTH=XOAUTH') && self._config.xoauth) {
      ...
      } else if (canSASL && self.serverSupports('AUTH=XOAUTH2')
                 && self._config.xoauth2) {
        self._caps = undefined;
        cmd = 'AUTHENTICATE XOAUTH2';
        //if (self.serverSupports('SASL-IR'))
          cmd += ' ' + escape(self._config.xoauth2);
        self._enqueue(cmd, checkCaps);
      } else if (!loginDisabled && self._config.user && self._config.password) {
        self._caps = undefined;
